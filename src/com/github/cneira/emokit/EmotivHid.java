...
final class EmotivHid implements Closeable {
    ...
    static {
	    ...
	    supportedResearch.add(new byte[]{33, -1, 31, -1, 30, 0, 0, 0});
            supportedResearch.add(new byte[]{32, -1, 31, -1, 30, 0, 0, 0});
            supportedResearch.add(new byte[]{0, 32, -1, 5, 101, 5, 101, 0});
        } catch (Exception e) {
        ...
        for (HIDDeviceInfo info : infos) {
            ...
            try {
              ...
              System.out.println("Found manufacturer" +  
                        dev.getManufacturerString() + " prod string" +
                        dev.getProductString() + "serial " +
                        dev.getSerialNumberString() + "report " +
